...
describe("c-customer-cases", () => {
  ...
  it("showing customer cases", () => {
    ...
    return Promise.resolve().then(() => {
      ...
      const caseDetails = element.shadowRoot.querySelectorAll("tbody tr");
      expect(caseDetails.length).toBe(mockGetCasesList.length);
      expect(caseDetails[0].textContent).toBe(
        mockGetCasesList[0].CaseNumber + mockGetCasesList[0].Subject +  mockGetCasesList[0].Origin +  mockGetCasesList[0].Status
      );
      
      console.log('caseDetails--->'+caseDetails.length);  
    });
  }); 
});
